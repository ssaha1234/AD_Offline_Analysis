...
# -----------------------------------------------------------------------------------------
# Define the custom loss function using the distance correlation (DisCo)
#def distance_corr(HLT_AD_scores, Offline_AD_scores, normedweight, power=1):
def distance_corr(HLT_AD_scores, Offline_AD_scores, power=1):

    # Ensure consistent data types
    # changed float 32--> 16 for saving memory
    HLT_AD_scores = tf.cast(HLT_AD_scores, tf.float16)
    Offline_AD_scores = tf.cast(Offline_AD_scores, tf.float16)

    xx = tf.reshape(HLT_AD_scores, [-1, 1])
    xx = tf.tile(xx, [1, tf.size(HLT_AD_scores)])
    xx = tf.reshape(xx, [tf.size(HLT_AD_scores), tf.size(HLT_AD_scores)])

    yy = tf.transpose(xx)
    amat = tf.math.abs(xx - yy)

    xx = tf.reshape(Offline_AD_scores, [-1, 1])
    xx = tf.tile(xx, [1, tf.size(Offline_AD_scores)])
    xx = tf.reshape(xx, [tf.size(Offline_AD_scores), tf.size(Offline_AD_scores)])
    ...
    minuend_1 = tf.tile(amatavg, [tf.size(HLT_AD_scores)])
    minuend_1 = tf.reshape(minuend_1, [tf.size(HLT_AD_scores), tf.size(HLT_AD_scores)])
    ...
    minuend_1 = tf.tile(bmatavg, [tf.size(Offline_AD_scores)])
    minuend_1 = tf.reshape(minuend_1, [tf.size(Offline_AD_scores), tf.size(Offline_AD_scores)])
    minuend_2 = tf.transpose(minuend_1)
...
#def loss_fn_DisCo(y_true, y_pred, HLT_AD_scores, Offline_AD_scores, normedweight, alpha=10.0):

def loss_fn_DisCo(y_true, y_pred, HLT_AD_scores, Offline_AD_scores, alpha=10.0):
    """Masked MSE with Distance Correlation (DisCo) with HLT_AD_scores."""
    
    # Ensure consistent data types (cast to float32--->16)
    ...
    # Replace 0s by 1s to avoid division by zero
    valid_count = tf.where(K.equal(valid_count, 0), tf.ones_like(valid_count), valid_count)
    
    # Calculate the mean squared error by dividing by the number of valid objects
    mean_squared_error = sum_squared_difference / valid_count

   # Ensure the mean squared error is float32-->16 to match the other tensors
    mean_squared_error = tf.cast(mean_squared_error, tf.float16)
      
   # Compute Distance Correlation (DisCo) between y_pred and HLT_AD_scores using the custom function
    #d_corr = distance_corr(HLT_AD_scores, Offline_AD_scores, normedweight)
    d_corr = distance_corr(HLT_AD_scores, Offline_AD_scores)
    
    # Calculate the final loss: MSE + alpha * DisCo loss
    total_loss = K.mean(mean_squared_error) + alpha * d_corr
...
def initialize_model_Offline(input_dim, dropout_p=0, L2_reg_coupling=0, latent_dim=4, large_network=True, saved_model_path=None, save_version=None):
    ...
    # Check the available datasets
        ...
        if group_name in f:
            eb_train_group = f[group_name]
            # Load the datasets, need to filter like before?
            HLT_AD_scores = eb_train_group['HLT_AD_scores'][:10000]  # (1048336,)
            Offline_AD_scores = eb_train_group['Offline_AD_scores'][:10000]
            print("HLT_AD_scores shape:", HLT_AD_scores.shape, HLT_AD_scores)
            print("Offline_AD_scores shape:", Offline_AD_scores.shape, Offline_AD_scores)
            
    
    #HLT_AD_scores = np.random.randn(1000, input_dim).astype(np.float32)  # Random HLT AD scores
    #Offline_AE.compile(optimizer='adam', loss=loss_fn, weighted_metrics=[])
    Offline_AE.compile(optimizer='adam', 
                  loss=lambda y_true, y_pred: loss_fn_DisCo(y_true, y_pred, 
                  HLT_AD_scores, 
                  Offline_AD_scores, 
                  #normedweight, 
                  alpha),
    ...
    if (saved_model_path is not None) and (save_version is not None):
        ...
        Offline_AE.load_weights(f'{saved_model_path}/EB_Offline_DisCo{save_version}.keras')
        Offline_encoder.load_weights(f'{saved_model_path}/EB_Offline_encoder_DisCo{save_version}.keras')
    # -------------------
...
def train_model(datasets: dict, model_version: str, save_path: str, dropout_p=0, L2_reg_coupling=0, latent_dim=4, large_network=True, training_weights=True):
    ...
    Offline_AE.save_weights(f'{save_path}/EB_Offline_DisCo{model_version}.keras')
    Offline_encoder.save_weights(f'{save_path}/EB_Offline_encoder_DisCo{model_version}.keras')
...
def process_multiple_models(training_info: dict, data_info: dict, plots_path: str, target_rate: int = 10, L1AD_rate: int = 1000, custom_datasets = None):
    ...
    output_file = os.path.join(output_dir, 'ad_scores_DisCo_10000_alpha100.h5')
        ...
        # Calculate the AD scores
            ...
            for tag, dict in datasets.items():
                ...
                #print('tag, dict', tag, datasets.items())
                 ...
                 #SS - This is where you get the AD scores from, you need to get the HLT_AD score to run DisCo
                 # Figure out the pathway, or load it in the function itself, for now
                 ##SS - This is calling 
